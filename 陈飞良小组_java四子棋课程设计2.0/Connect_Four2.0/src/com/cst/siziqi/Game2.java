...
public class Game2 extends JFrame implements KeyListener, ActionListener,MouseListener
{
	//组件声明
	private final int N = 4;
	private JPanel MainPanel2 = null;
	...
	private void InitLabel()
	{
		...
		// 主面板（棋盘）
		this.MainPanel2 = new MainPanel2();
		this.MainPanel2.setBounds(0, 0, 800, 600);
		this.MainPanel2.setLayout(null);
		MainPanel2.addMouseListener(this);
		con.add(this.MainPanel2);
		...
		for (int i = 0; i < 7; i++)
		{
			for (int j = 0; j < 6; j++)
			{
				...
				this.player[i][j] = new JLabel(new ImageIcon("./img/player2.png"));
				this.player[i][j].setBounds(285 + 65 * i, 127 + 65 * j, 65, 65); // x轴上每个棋子相差36，边框相差63，y轴上每个棋子相差36，边框相差64。
				
				this.player[i][j].setVisible(false);// 不可见
				this.MainPanel2.add(this.player[i][j]);

				this.computer[i][j] = new JLabel(new ImageIcon("./img/computer2.png"));
				this.computer[i][j].setBounds(285 + 65 * i, 127 + 65 * j, 65, 65);
				this.computer[i][j].setVisible(false);
				this.MainPanel2.add(this.computer[i][j]);
		...
		this.choose.setBounds(283,45, 80, 80);
		this.MainPanel2.add(this.choose);
	...
	private void initTime()
	{
		...
		this.time.setBounds(440, 540, 80, 25);
		this.time.setFont(font);
		this.setPreferredSize(new Dimension (100,100));
		MainPanel2.add(this.time);	
	...
	{
		this.choose.setLocation(283 + 65 * this.choose.getXX(), 45 + 65 * this.choose.getYY());
	}
			...
			case KeyEvent.VK_SPACE:
				...
				{ 
					if (this.pos[this.choose.getXX()][j].getLabel_Status() == 0 && this.choose.getXX() < 7&& this.choose.getXX() >= 0)
					{
						...
						this.choose.setLocation(283 + 65 * this.choose.getXX(), 45);
						this.repaint();
	...
	public void mouseClicked(MouseEvent e)
	{
		...
		{ 
			if (this.pos[x][j].getLabel_Status() == 0 && x < 7&& x >= 0)
			{
				...
				this.choose.setLocation(283 + 65 * x, 45 );
				this.repaint();
